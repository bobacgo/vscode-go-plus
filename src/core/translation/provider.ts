...
export class TranslationProvider implements vscode.CodeActionProvider {
    ...
    private static readonly decorationType = vscode.window.createTextEditorDecorationType({
        after: {
            margin: '0 0 0 1em',
            contentText: '翻译',  // 初始为空，会在设置装饰器时动态设置
            fontStyle: 'italic',
    ...
    private config = {
        ...
        targetLang: 'zh-CN',
        autoDetect: true,
        engineType: TranslationService.ENGINE_TYPES.BUILT_IN
    };
    ...
    private loadConfig(): void {
        ...
        this.config = {
            ...
            engineType: config.get('engineType', TranslationService.ENGINE_TYPES.BUILT_IN)
        };
    }
    ...
    private handleConfigChange(event: vscode.ConfigurationChangeEvent): void {
        if (event.affectsConfiguration('goAssist.translation')) {
            this.loadConfig();
        }
    }
    ...
    public async translateVisibleComments(dontClearDecorations = false): Promise<void> {
        ...
        try {
            ...
            logger.debug(`找到 ${comments.length} 条注释 / Found ${comments.length} comments`);
            
            if (comments.length === 0) {
                if (!dontClearDecorations) {
                    vscode.window.showInformationMessage('没有找到需要翻译的注释 / No comments found to translate');
                }
                return;
            }
